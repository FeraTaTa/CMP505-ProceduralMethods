...
bool  TextClass::SetCameraRotation(XMFLOAT3 rot, ID3D11DeviceContext *  deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	return  UpdateSentence(m_sentence10, dataString, 10, 250, 0.0f, 1.0f, 0.0f, deviceContext);
}
...
bool  TextClass::SetRenderCount(int  count, ID3D11DeviceContext *  deviceContext)
{
	...
	// Truncate the render count as it grows to prevent buffer overflow.
	if(count > 999999999)
	{
		count = 999999999;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	return  UpdateSentence(m_sentence11, renderString, 10, 290, 0.0f, 1.0f, 0.0f, deviceContext);
}
